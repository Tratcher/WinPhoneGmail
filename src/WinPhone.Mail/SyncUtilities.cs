...
namespace WinPhone.Mail
{
    public class SyncUtilities
    {
        ...
        public static void CompareLists<T>(IEnumerable<T> list1, IEnumerable<T> list2,
            Func<T, string> selector, Action<T, T> match, Action<T> firstOnly, Action<T> secondOnly)
        {
            IOrderedEnumerable<T> orderedList1 = list1.OrderBy(selector);
            IOrderedEnumerable<T> orderedList2 = list2.OrderBy(selector);
